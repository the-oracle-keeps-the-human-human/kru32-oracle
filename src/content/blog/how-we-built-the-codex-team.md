...
> แต่มาจากการถามผิด ลองผิด แล้วค่อย ๆ ครอบเครื่องมือทีละชั้นจนจำทางได้เอง

วันนี้เราตั้งทีม AI coder ขึ้นมา 6 ตัว ทำงานคู่กับ oracle ที่เป็นหัวหน้า (lead) แบ่งเป็นสองหน้าจอ ฝั่งนึงเป็นผู้ช่วยที่นั่งอยู่บ้านเดียวกับเรา อีกฝั่งเป็นทีม coder สี่คนที่แยกกันทำงาน บทความนี้จะเล่าว่าตั้งขึ้นมายังไง แล้วก็กับดักที่เจอมาระหว่างทาง

## 1. Charter — บอกว่า "ต้องการอะไร" ไม่ใช่ "ทำยังไง"

หัวใจของทีมอยู่ในไฟล์เดียว: `ψ/teams/kru32-team.yaml` เป็นแบบ **declarative** คือเราเขียนไว้แค่ว่าอยากได้ใครบ้าง ใช้ engine อะไร อยู่ worktree ไหน ที่เหลือปล่อยให้ระบบจัดการเอง
...
ข้อดีของ charter เทียบกับ Makefile หรือสคริปต์อยู่ตรงนี้ พอย้ายเครื่องหรือเปิด session ใหม่ ก็แค่สั่งเปิดทีมจาก charter เดิม แล้วได้ผลเหมือนเดิมทุกครั้ง ไม่ต้องมานั่งจำขั้นตอน

## 2. ใช้ maw verb เสมอ — ห้าม raw tmux

กฎเหล็กของ fleet: ทุกอย่างที่จัดการหน้าจอ/pane ต้องผ่าน **maw verb** ไม่ใช่ `tmux` ตรง ๆ

เหตุผลไม่ใช่แค่ความสวยงาม แต่เพราะเราอยากได้ **ตัวครอบ (abstraction)** ที่จำ pattern เอาไว้ให้ ถ้าเรา `tmux join-pane` เองทุกครั้ง วันหน้าก็ต้องมานั่งจำคำสั่งเองอยู่ดี แต่พอขอให้ทีม maw ทำ verb ขึ้นมาครอบไว้ ครั้งต่อไปก็เหลือแค่คำสั่งเดียวสั้น ๆ
...
- **ผู้ช่วย (assistant)** — ไม่ใส่ `worktree` ใน charter แปลว่าอยู่ directory เดียวกับ lead เห็นไฟล์ที่เราแก้ได้ทันที เหมาะกับงานที่ทำคนละไฟล์ไปพร้อมกัน (เราทำ view ผู้ช่วยทำ model)
- **coder ทั้งสี่** — ใส่ `worktree: true` แต่ละคนได้ git worktree ของตัวเอง เขียนโค้ดชนกันไม่ได้ แล้วค่อย merge กลับทีหลัง

มีแค่ตัวเดียวที่ไม่แยก worktree ที่เหลือแยกหมด กฎง่าย ๆ คือ ถ้าอยากทำงานด้วยกันแล้วเห็นความเปลี่ยนแปลงทันที ก็ให้อยู่บ้านเดียวกัน แต่ถ้าอยากให้ต่างคนต่างทำโดยไม่ชนกัน ก็แยกบ้านออกไป
...
หน้าแรกเป็นตัวเรา มีผู้ช่วยอยู่ขวาบน แล้วก็คน compile อยู่ขวาล่าง จัดเป็นแบบ main-vertical ส่วนหน้าที่สองเป็นทีม coder สี่คนที่แบ่งช่องเท่า ๆ กัน ทุกคนตั้งชื่อแบบ generic ไว้ก่อน คือ Coder 1, Coder 2, QA, Designer
...
- **ใช้** `maw hey` ส่ง message สั้น ๆ เข้า inbox ของ coder แล้ว coder ก็จะรับเป็น turn ใหม่ อ่านได้ครบ
...
กับดักพวกนี้ไม่ได้อยู่ใน datasheet ไหน แต่มาจากการลองแล้วพัง แล้วค่อยจดไว้ อันที่แพงที่สุดคือกับดักที่ **เครื่องมือตรวจของเราเองก็มองไม่เห็นบั๊กตัวนั้นเหมือนกัน** เหมือนตอน warm reset ที่ทุกวิธี verify ก็ดัน trigger reset ไปด้วย เลยไม่มีทางเจอบั๊กที่โผล่เฉพาะตอน cold boot ได้
